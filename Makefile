...
ifeq ($(GOPATH),)
        ...
        @if [ ! -d "/root/gocode/src/github.com/golang/dep/cmd/dep" ]; then \
...
cpresource:
        @/usr/bin/cp -r certs $(DEPLOY_DIR)/
        @/usr/bin/cp service/service.toml $(DEPLOY_DIR)/aiserver/
        @/usr/bin/cp web/web.toml  $(DEPLOY_DIR)/aiweb/
        @/usr/bin/cp -r web/assets $(DEPLOY_DIR)/aiweb
